...
Completion receipts are bound to the resolved participant. Guided or expert human work accepts only a human receipt; model work accepts only the exact candidate, provider, model, qualification ID, and qualification-bundle hash. Contract-only work may use a human or an unbound harness. Git source records separately retain the repository object format and exact blob ID from the normalized-text SHA-256 digest. Downstream definitions remain scoped to their declared requirement, criterion, or journey and never establish execution or data accuracy.
