...
External-validation methodology and its reusable execution substrate are documented in [external_validation/README.md](external_validation/README.md). The tracked public Session 1 resolver reports detection `QUALIFIED`, remediation `BLOCKED`, and overall `PARTIALLY_QUALIFIED` until the private root-owned closeout attestation is supplied; a valid authorized resolution reports all three profiles `QUALIFIED`. The control plane is separate from private patient repositories, hidden oracles, and raw execution evidence.
...
Session 5 production-boundary closure invokes all 27 portable contracts through their real validators or stateful loaders. Its parity reports distinguish structural rejection from semantic rederivation, and separately audit the private qualification rubric. The executable closeout report binds those reports, final-run JUnit evidence, real applicable artifacts, the final commit, and a self-hash. Passing totals or symbol existence alone do not close the session. Measurement & AI domain code rejects model, command, network/browser, SQL, warehouse/BI, tracing-service, and external-eval execution and writes only beneath its ignored release root or qualification store.
