...
def validate_real_use_qualification_serialized(raw: bytes, binding_raw: bytes, adjudication_raw: bytes) -> dict[str, Any]:
    ...
    if value.get("implementation_binding") != binding or value.get("adjudication_root") != adjudication["authority_bindings"]["review_root"] or value.get("adjudication_projection_sha256") != sha256_bytes(adjudication_raw) or adjudication.get("live_evaluation",{}).get("implementation_commit")!=binding.get("implementation_commit"):
