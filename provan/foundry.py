...
PROVIDERS = {
    "openai-responses-primary":{"origin":"https://api.openai.com","model":"gpt-5.2","qualified_roles":["semantic_interpreter","strong_reasoner","independent_critic"],"store_requested":False,"retention":"PROVIDER_RETENTION_NOT_ZERO_OR_ESTABLISHED"},
...
def route(risk: dict[str, Any]) -> dict[str, Any]:
    required = {"risk", "ambiguity", "blast_radius", "reversibility", "oracle", "actor_autonomy"}
    if set(risk) != required or any(value == "unresolved" for value in risk.values()): tier = 3
...
def _proposal(brief: dict[str, Any], sources: list[dict[str, Any]], interpretation: str) -> dict[str, Any]:
    ...
    outcome = intent[0][:4096] if intent else "INTENDED_OUTCOME_UNRESOLVED"
...
def foundry(*, brief_id: str, source_manifest: Path, interpretation: str = "faithful", depth: str = "standard", provider_id: str | None = None, no_model: bool = False, format_name: str = "terminal") -> tuple[dict[str, Any], str]:
    ...
    proposal = _proposal(brief, sources, interpretation)
    blind_paths = [];envelope_refs=[];usage_receipts=[]
    ...
    if semantic_available and semantic_path_count:
        for label in (("A","B") if semantic_path_count==2 else ("A",)):
            ...
            provider=ModelProvider(provider_id,PROVIDERS[provider_id]["model"],"pinned-work-order-v1",PROVIDERS[provider_id]["origin"])
            blocks=[{"category":f"blind_{row['role']}","content":row["content"]} for row in sources if row["role"] in {"intent","formal_contract"}]
            envelope=build_envelope(case_id=case_id,candidate_digest=brief["candidate"]["candidate_digest"],provider=provider,instructions=instructions,blocks=blocks);envelope["prompt_id"]=f"foundry-deep-path-{label.lower()}";envelope_raw=canonical_bytes(envelope);_schema("model-input-envelope.v1.json",envelope);path=f"model-input-envelope-{label.lower()}.json";secure_write(root/path,envelope_raw);envelope_ref={"id":envelope["envelope_id"],"path":path,"sha256":sha256_bytes(envelope_raw),"prompt_id":envelope["prompt_id"]};envelope_refs.append(envelope_ref)
            ...
            else:result={"model_reviewed_implications":[proposal["intended_outcome"]],"unresolved":["SCRIPTED_PROVIDER_SEMANTICALLY_UNQUALIFIED"]};usage={"mode":"SCRIPTED_TEST","calls":1,"latency_ms":0,"cost_status":"not_applicable","envelope_digest":sha256_bytes(envelope_raw)}
    ...
    intent_id=str(uuid.uuid4());intent={"schema_id":"provan.intent_model.v1","intent_id":intent_id,"path_id":"deterministic" if not blind_paths else "synthesis","source_ledger_ref":ledger_ref,"outcomes":[proposal["intended_outcome"]],"users":[],"journeys":[],"ambiguities":["TARGET_USER_UNRESOLVED","EXACT_ORACLE_REQUIRES_OWNER_CONFIRMATION"],"non_goals":["runtime verification","challenge execution"],"authority":"model_reviewed_proposal" if semantic_available else "source_attributed_proposal","limitations":limitations};intent_ref,_=_stage(root,"intent-model",intent,"intent-model.v1.json","intent_id")
    goal_id=str(uuid.uuid4());goal={"schema_id":"provan.goal_obstacle_model.v1","model_id":goal_id,"intent_ref":intent_ref,"goals":[proposal["intended_outcome"]],"obstacles":["owner confirmation outstanding","runtime evidence unavailable"],"unknowns":["target user","final oracle"],"authority":"proposal_only","limitations":limitations};goal_ref,_=_stage(root,"goal-obstacle-model",goal,"goal-obstacle-model.v1.json","model_id")
