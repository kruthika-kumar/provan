...
def foundry(*, brief_id: str, source_manifest: Path, interpretation: str = "faithful", depth: str = "standard", provider_id: str | None = None, no_model: bool = False, format_name: str = "terminal", information_boundary: str | None = None, view: str = "full") -> tuple[dict[str, Any], str]:
    ...
    if semantic_available and semantic_path_count:
        for label in (("A","B") if semantic_path_count==2 else ("A",)):
            ...
            else:result={"model_reviewed_implications":["Scripted proposal only: "+_source_outcome(sources)],"unresolved":["SCRIPTED_PROVIDER_SEMANTICALLY_UNQUALIFIED"]};usage={"mode":"SCRIPTED_TEST","calls":1,"latency_ms":0,"cost_status":"not_applicable","envelope_digest":sha256_bytes(envelope_raw)}
    ...
    synthesized_outcome=_source_outcome(sources)
    ...
    stage_execution=[];source_inputs=[ledger_ref["sha256"]];previous=source_inputs
    for stage_name in RUN_STAGES[depth]:
        outputs=stage_outputs[stage_name];status="EXECUTED" if outputs else "NOT_APPLICABLE"
        if depth == "deep" and stage_name in {"blind_path_a", "blind_path_b"}:
            inputs = source_inputs
        elif depth == "deep" and stage_name == "freeze_blind_paths":
            inputs = path_digests
        else:
            inputs = previous
        stage_execution.append({"stage":stage_name,"input_digests":inputs,"output_digests":outputs,"status":status})
