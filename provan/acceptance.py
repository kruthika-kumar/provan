...
def _contract_obligation_items(contract:dict[str,Any],seed_digest:str,actor:dict[str,Any],timestamp:str)->list[dict[str,Any]]:
    snapshot={key:contract[key] for key in CONTRACT_OBLIGATION_FIELDS};proposals=[("contract_obligation","contract:obligation:v1",snapshot)]
    ...
    for kind,source_ref,original_value in proposals:
        rows.append({"item_id":_item_id(seed_digest,kind,source_ref,original_value),"kind":kind,"source_ref":source_ref,"original_value":original_value,"action":"confirm","edited_value":None,"rationale":"explicit case-operator contract-surface disposition","actor":actor,"acted_at":timestamp})
    return rows
