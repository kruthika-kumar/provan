...
from .session11_validators import (
    ...
    validate_reinspection_serialized, validate_settlement_serialized,
)
...
def _make_freeze(contract:dict[str,Any],contract_raw:bytes,repo_source:str,*,purpose:str,head_override:str|None,now:Callable[[],datetime]) -> dict[str,Any]:
    ...
    verification={r["path"]:r.get("static_details",{}).get("content_digest") for r in analysis["changed_files"] if any(x in r["surface_classes"] for x in ("verification_surface","ci_configuration","test_source"))}
...
def _record_data(att:dict[str,Any],att_raw:bytes,settlement:dict[str,Any],decision:dict[str,Any]|None,decision_raw:bytes|None,record_id:str,*,now:Callable[[],datetime])->dict[str,Any]:
    open_rows=[r for r in settlement["criteria"] if r["state"] not in {"established","not_applicable"}];return {"record_id":record_id,"record_contract":"provan.acceptance_record.v1","record_version":1,"attestation_ref":_ref(att,att_raw,"attestation_id"),"decision_ref":_ref(decision,decision_raw,"decision_id") if decision and decision_raw else None,"subject":att["subject"],"recommendation":att["recommendation"],"owner_decision":decision and decision["decision"],"effective_status":effective_status(att.get("expires_at"),now),"priority_open_items":open_rows[:3],"additional_open_count":max(0,len(open_rows)-3),"criteria":settlement["criteria"],"limitations":["SESSION12_VERIFIER_EXECUTION_UNAVAILABLE","SESSION13_CHALLENGE_EXECUTION_NOT_RUN"],"conflict_of_interest":"Internal Provan dogfood may use explicit founder/operator confirmation; recommendation remains evidence-derived."}
...
def render_record(attestation_id:str,decision_id:str|None,format_name:str,*,now:Callable[[],datetime]=utcnow)->tuple[str,str]:
    ...
    views={name:{"path":f"{name}.txt" if name=="terminal" else f"record.{name}","sha256":sha256_bytes(text.encode())} for name,text in rendered.items()}
