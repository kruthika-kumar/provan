...
def test_proof_final_artifact_binding(binding_kind,fixture_class):
    ...
    if binding_kind=="candidate-freeze":
        ...
        if valid:
            ...
            valid=candidate.get("organisation_identity_asserted") is False and candidate.get("recommendation")=="held" and candidate.get("owner_decision")=="hold" and any(name.startswith("closure_requirement_") for name in artifacts) and candidate.get("candidate")=={key:brief["candidate"][key] for key in ("repository_identity","base","head","mode")} and contract["candidate"]==brief["candidate"] and freeze["candidate"]==brief["candidate"] and decision["decision"]==candidate["owner_decision"] and record["record_id"]==candidate["record_id"] and candidate["implementation_commit"]==brief["candidate"]["head"] and candidate["wheel_sha256"]=="sha256:9f169a72eff8e93af97e039ed11d4a90f47263057c9f8bbc5050a7891c1d4bcd"
