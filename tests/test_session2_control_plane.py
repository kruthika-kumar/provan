...
from shiproom.external_validation.session2 import (BudgetLedger, BudgetPolicy,
    ...
    validate_fresh_b_retrieval_frame, validate_fresh_b_retrieval_frame_v2)
...
def test_fresh_b_population_authority_freezes_only_b1_b2_b3():
    value = json.loads((Path("external_validation/manifests/session2/fresh_b_population_authority.v1.json")).read_text())
    assert validate_fresh_b_population_authority(value)["band_order"] == ["B1", "B2", "B3"]
...
def test_fresh_b_recovery_frame_partitions_the_same_fix_population():
    value = json.loads(Path("external_validation/manifests/session2/fresh_b/recovery/healthchecks_retrieval_frame.v2.json").read_text())
    assert validate_fresh_b_retrieval_frame_v2(value)["page_size"] == 100
    value["retrieval_not_before"] = "not-a-time"
    with pytest.raises(Session2ValidationError, match="session2_fresh_b_retrieval_frame_invalid"):
...
def test_linux_primary_retrieval_seals_exact_raw_pages_without_selecting_cases(tmp_path: Path, monkeypatch):
    ...
    pull["items"][0]["closed_at"] = "2026-05-02T00:00:00Z"
    assert retrieval._candidate_ids(pull, {"kind": "pull_request", "merged_from": "2026-03-01T00:00:00Z", "merged_to": "2026-04-30T00:00:00Z"}) == ["acme/project#8"]
...
def test_fresh_b_finalizer_uses_authoritative_pr_merged_at_and_preserves_source_only_status(tmp_path: Path, monkeypatch):
    ...
    reference = {"schema_id":"external_validation.session2_fresh_b_candidate_reference_index.v1","schema_version":"1","retrieval_frame_receipt_hashes":["sha256:" + "11" * 32],"source_receipt_hashes":["sha256:" + "22" * 32],"object_receipt_not_before":"2026-07-30T10:46:32.627787Z","selection_effect":"reference_collection_only","candidates":[{"candidate_id":"acme/project#7->acme/project#8","source_priority":2,"repository":"acme/project","issue_number":7,"fix_repository":"acme/project","fix_pr_number":8,"issue_created_at":"2026-02-20T00:00:00Z","search_fix_closed_at":"2026-03-30T00:00:00Z","fresh_b_band":"B1","contamination_tier":"FRESH_B","issue_retrieval_receipt_hash":"sha256:" + "33" * 32,"fix_retrieval_receipt_hash":"sha256:" + "44" * 32,"selection_status":"REFERENCE_ONLY_REQUIRES_OBJECT_RECEIPTS"}],"exclusions":[]}
...
def test_session2_mirror_seals_failed_fetch_and_requires_a_new_attempt_namespace(tmp_path: Path, monkeypatch):
    ...
    assert any("--depth=1" in invocation for invocation in observed)
