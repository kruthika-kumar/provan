...
def test_node_yarn_capability_gap_is_sealed_without_reclassifying_the_lock(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(environment.os, "fchown", lambda *_args: None, raising=False)
    monkeypatch.setattr(environment.os, "fchmod", lambda *_args: None, raising=False)
    ...
    record = __import__("json").loads(next(store.glob("*.environment-build-failure.json")).read_text())
    assert record["failure_stage"] == "NODE_RUNTIME_CAPABILITY"
    assert record["failure_code"] == "session2_environment_node_runner_unqualified"
