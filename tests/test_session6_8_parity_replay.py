...
def test_every_inventory_contract_has_replayable_structural_and_semantic_mutation(tmp_path):
    ...
    result=validate(report);assert result=={"schema_version":"session6-8-contract-parity-validation.v2","contract_count":53,"mutation_count":106,"unexpected_pass_count":0,"status":"passed"}
    value=__import__("json").loads(report.read_text())
    assert len(value["accepted_baselines"])==53
