...
def test_basis_authority_and_reviewer_authority_are_not_collapsed():
    ...
    with pytest.raises(ValueError,match="mixed deterministic and source-backed"):
        effective_basis_class(["source_verified", "deterministically_established"])
...
def test_overlay_exact_schema_and_reference_validation():
    value = {
        "schema_version":"measurement-ai-overlay.v2","release_id":"rel","release_commit":"a"*40,
        "product_intent_semantic_hash":"sha256:"+"1"*64,"graph_semantic_hash":"sha256:"+"2"*64,
        "nodes":[{"node_id":"contract_1","node_type":"measurement_contract","provenance":"measurement_ai_compiler","contract_id":"contract_1","journey_id":None,"criterion_ids":["criterion_1"]}],
        "edges":[{"edge_id":"edge_1","source_node_id":"contract_1","target_node_id":"criterion_1","relationship":"governs_criterion","direct_fact_authority":"source_verified","criterion_id":"criterion_1","criterion_path":[{"edge_id":"edge_1","traversal":"forward"}],"criterion_basis_authority":"source_verified","origin":"prepared","references":[]}],
    }
...
def test_foundation_json_schemas_parse():
    for name in ("measurement-ai-role.v2.json", "work-order.v5.json", "measurement-ai-overlay.v2.json"):
...
def test_instrumentation_requirement_issues_only_measurement_role(tmp_path):
    ...
    schema=json.loads(resources.files("shiproom.measurement_ai_schemas").joinpath("work-order.v5.json").read_text())
    jsonschema.Draft202012Validator(schema).validate(loaded["work_orders"]["measurement"])
    role_schema=json.loads(resources.files("shiproom.measurement_ai_schemas").joinpath("measurement-ai-role.v2.json").read_text())
    jsonschema.Draft202012Validator(role_schema).validate(json.loads(resources.files("shiproom.measurement_ai_roles").joinpath("measurement.v2.json").read_text()))
...
def place_result(ctx, preparation_id, role, record):
    ...
    record["basis_ids"]=[bases[0]["basis_id"]]; record["basis_path_ids"]=[]
    value={"schema_version":"measurement-result.v2" if role=="measurement" else "ai-evaluation-result.v2","role_id":role,"role_version":"2.0.0","preparation_id":preparation_id,"work_order_id":work["work_order_id"],"base_graph_semantic_hash":work["inputs"]["graph_semantic_hash"],"resolved_review_mode":work["resolved_review_mode"],"records":[record],"recommendations":[],"assumptions":[],"limitations":[]}
...
def test_skip_generation_has_all_six_not_applicable_checks(tmp_path):
    ctx=conventional_context(tmp_path); prep=prepare(ctx); manifest=compile_generation(ctx,prep["preparation_id"]); assert manifest["compiler_version"]=="portable-measurement-ai.v2"
...
def test_v2_result_schema_and_python_both_reject_nested_extra_fields(tmp_path):
    ctx=assessment_context(tmp_path); root=domain_root(ctx)/"inputs"; root.mkdir(parents=True); app=default_applicability(); graph=load_assessment_input(ctx)["graph_artifacts"]["requirement-evidence-graph.json"]; cid=next(item["node_id"] for item in graph["nodes"] if item["node_type"]=="acceptance_criterion"); app["measurement"]["criterion_ids"]=[cid]; path=root/"applicability.json"; path.write_text(json.dumps(app),encoding="utf-8"); info=prepare(ctx,applicability_path=str(path)); place_result(ctx,info["preparation_id"],"measurement",assessed_record(cid)); prep=load_preparation(ctx,info["preparation_id"]); work=prep["work_orders"]["measurement"]; inbox=domain_root(ctx)/"inbox"/info["preparation_id"]/work["work_order_id"]; value=json.loads((inbox/"result.json").read_text()); value["records"][0]["metric_dimensions"].append({"dimension":"population","state":"adequate","rationale":"bounded","extra":True}); raw=(json.dumps(value)+"\n").encode(); schema=json.loads((prep["directory"]/"contract-schemas"/"measurement-result.v2.json").read_text())
...
def test_staged_verifier_binds_all_primary_hashes(tmp_path):
    ...
    info=prepare(ctx,review_mode="expert_escalated_review",review_capabilities={"schema_version":"measurement-review-capabilities.v2","executor_type":"human","reviewer_label":"expert"},permission={"schema_version":"measurement-review-permission.v2","release_id":ctx.release["release_id"],"expert_review_granted":True,"model_switch":{"decision":"not_requested"}},applicability_path=str(path)); prep=load_preparation(ctx,info["preparation_id"]); record=assessed_record(cid); record["semantic_review_authority"]="dual_reviewed_with_curated_guidance"; place_result(ctx,info["preparation_id"],"measurement",record); work=prep["work_orders"]["measurement"]; inbox=domain_root(ctx)/"inbox"/info["preparation_id"]/work["work_order_id"]; value=json.loads((inbox/"result.json").read_text()); basis=value["records"][0]["basis_ids"]; value["recommendations"]=[{"local_id":"warning","criterion_id":cid,"recommendation_class":"research_backed_warning","summary":"Opportunity may vary.","basis_ids":basis,"basis_path_ids":[],"guidance_rule_ids":["MEAS_COUNT_002"],"exception_dispositions":[{"exception_id":item,"disposition":"unknown","basis_ids":[]} for item in ("absolute_volume_decision","fixed_opportunity","business_outcome_with_diagnostics","no_incremental_claim")],"requested_effect":"non_blocking_warning","abstained":True,"automatic_replacements":[]}]; raw=(json.dumps(value,sort_keys=True)+"\n").encode(); (inbox/"result.json").write_bytes(raw); receipt=json.loads((inbox/"completion-receipt.json").read_text()); receipt["result_snapshot_hash"]=sha256_bytes(raw); (inbox/"completion-receipt.json").write_text(json.dumps(receipt))
...
def test_qualification_is_mechanically_graded_and_hash_bound():
    ...
    value={"schema_version":"measurement-reviewer-qualification-result.v2","task_id":task["task_id"],"task_hash":task["task_hash"],"provider_id":"configured","model_id":"qualified","case_results":cases}
