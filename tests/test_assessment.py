...
def test_installed_wheel_prepares_assessment_outside_source_checkout(tmp_path: Path):
    ...
    closure_contract=json.loads((remediation_root/"generations"/remediation_pointer["generation"] / "closure-contracts" / (closure_id+".json")).read_text(encoding="utf-8"))
    ...
    outcome=lambda check_id:{"check_id":check_id,"passed":True,"evidence_class":"deterministically_established"}
    evidence = {"schema_version":"remediation-closure-evidence.v1","closure_contract_id":closure_id,"release_id":ctx.release["release_id"],"release_commit":ctx.authority_binding["repository_commit"],"branch":branch,"fixer_id":"wheel_fixer","reruns":[{"check_id":packet["source_issue_id"],"passed":True,"evidence_class":"deterministically_established"}],"regression_results":[outcome(item) for item in closure_contract["regression_checks"]],"test_results":[outcome(item) for item in closure_contract["test_requirements"]],"instrumentation_results":[outcome(item) for item in closure_contract["instrumentation_requirements"]],"protected_invariant_outcomes":[{"invariant":"canonical_findings_unchanged","passed":True}]}
