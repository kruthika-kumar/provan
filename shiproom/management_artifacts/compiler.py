...
def compile(ctx:LocalExecutionContext)->dict:
    vector=dependency_vector(ctx);policy=_policy(ctx,vector);generation="gen_"+uuid.uuid4().hex;directory=ensure_directory(ctx.repository_root,root(ctx)/"generations"/generation,label="management_generation")
    ...
    artifacts={
      ...
      "measurement-ai-readiness":{**base,"sections":_sections("measurement-ai-readiness"),"authority_note":"Canonical Measurement & AI authority is not recalculated by reporting."},
      "remediation-overview":{**base,"sections":_sections("remediation-overview"),"remediation_dependency":vector["remediation"]},
      "release-packet-index":{**base,"sections":_sections("release-packet-index"),"artifacts":list(JSON_ARTIFACTS)},
      "release-recommendation-view":{**base,"sections":["computed_recommendation","canonical_finding_state","owner_decision_state","accepted_conditions","unknowns","source_generations"],"computed_recommendation":policy,"canonical_finding_state":ctx.release.get("findings",[]),"owner_decision_state":ctx.release.get("owner_decisions",[])}}
