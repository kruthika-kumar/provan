...
def _write_once(directory: Path, raw: bytes) -> str:
    ...
    try: fd = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_CLOEXEC | getattr(os,"O_NOFOLLOW",0), 0o400)
...
def acquire_pair(repo: Path, *, candidate_id: str, repository: str, base_sha: str, head_sha: str, source_receipts: list[str]) -> dict[str,str]:
    ...
    if init.returncode: _fail("session2_mirror_init_failed")
    ...
    completed = _utc()
    if fetch.returncode: _fail("session2_mirror_fetch_failed")
    verified = []
...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--repository-root",type=Path,required=True); p.add_argument("--candidate-id",required=True); p.add_argument("--repository",required=True); p.add_argument("--base-sha",required=True); p.add_argument("--head-sha",required=True); p.add_argument("--source-object-receipt-hash",action="append",required=True); a=p.parse_args()
    try: print(json.dumps(acquire_pair(a.repository_root,candidate_id=a.candidate_id,repository=a.repository,base_sha=a.base_sha,head_sha=a.head_sha,source_receipts=a.source_object_receipt_hash),sort_keys=True))
