...
def node_runtime_unqualified(repository: Path, *, snapshot: Path, project_name: str,
    ...
    if (not isinstance(package_value, dict) or not lock_bytes.startswith(b"# THIS IS AN AUTOGENERATED FILE")
            or b"yarn lockfile v1" not in lock_bytes[:256]):
    ...
    failure = {
        ...
        "yarn_lock_hash": _sha(lock_bytes), "yarn_authority_path": yarn_authority_path,
        "patient_network_policy": "none",
    }
    _, digest = _write_once(receipts, ".environment-build-failure.json", canonical_json(failure))
