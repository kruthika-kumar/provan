...
DOCKERD=$(command -v dockerd); DOCKER=$(command -v docker); PYTHON=$(command -v python3); TAIL=$(command -v tail)
[[ -x "$DOCKERD" && -x "$DOCKER" && -x "$PYTHON" && -x "$TAIL" && -r "$DIR/bounded-log.py" ]] || die executables
...
"$TAIL" -f /dev/null 9>&- >"$LOG_FIFO" 2>/dev/null & keeper_pid=$!
