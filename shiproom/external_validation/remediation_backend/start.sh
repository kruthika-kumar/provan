...
DOCKERD=$(command -v dockerd); DOCKER=$(command -v docker); PYTHON=$(command -v python3); TAIL=$(command -v tail); SETSID=$(command -v setsid)
[[ -x "$DOCKERD" && -x "$DOCKER" && -x "$PYTHON" && -x "$TAIL" && -x "$SETSID" && -r "$DIR/bounded-log.py" ]] || die executables
[[ "$(hash "$DAEMON_JSON")" == "$(state_get CONFIG_HASH)" ]] || die daemon_config
daemon_pid=; launcher_pid=; logger_pid=; keeper_pid=
rollback(){ local rc=$? target=${daemon_pid:-${launcher_pid:-}}; trap - EXIT; if [[ -n "$target" && -r /proc/$target/stat ]]; then kill -TERM "$target" 2>/dev/null || true; for _ in $(seq 1 10); do kill -0 "$target" 2>/dev/null || break; sleep 1; done; kill -0 "$target" 2>/dev/null && kill -KILL "$target" 2>/dev/null || true; fi; [[ -n ${keeper_pid:-} ]] && kill -TERM "$keeper_pid" 2>/dev/null || true; [[ -n ${logger_pid:-} ]] && kill -TERM "$logger_pid" 2>/dev/null || true; rm -f "$SOCKET" "$PID" "$LOG_FIFO"; exit "$rc"; }; trap rollback EXIT INT TERM
...
"$SETSID" "$PYTHON" "$DIR/bounded-log.py" --input "$LOG_FIFO" --output "$LOG" --maximum 1048576 9>&- & logger_pid=$!
"$SETSID" "$TAIL" -f /dev/null 9>&- >"$LOG_FIFO" 2>/dev/null & keeper_pid=$!
...
if [[ "$TEST_MODE" != 1 ]]; then chown root:root "$SOCKET"; fi; chmod 0600 "$SOCKET"; state_put DAEMON_PID "$daemon_pid"; state_put DAEMON_START "$(pid_start "$daemon_pid")"; state_put DOCKERD_EXE "$(readlink -f "$DOCKERD")"; state_put DOCKER_CLI "$(readlink -f "$DOCKER")"; state_put LOG_PID "$logger_pid"; state_put LOG_START "$(pid_start "$logger_pid")"; state_put LOG_KEEPER_PID "$keeper_pid"; state_put LOG_KEEPER_START "$(pid_start "$keeper_pid")"; state_put LOG_KEEPER_EXE "$(readlink -f "$TAIL")"
daemon_verified || die daemon_identity; log_pipeline_verified || die logger_identity
