#!/usr/bin/env bash
set -Eeuo pipefail; DIR=$(cd "$(dirname "$0")"&&pwd); source "$DIR/lib.sh"; root_only; privileged_entry_guard; [[ ${1:-} == --locked ]] || with_lock; require_paths; control_ready; daemon_verified || die daemon_identity; logger_probe || die logger_identity; storage_verified || die quota_storage
