...
install -d -m 0700 "$MOUNT/docker-data" "$MOUNT/worktrees" "$MOUNT/quarantine"; xfs_quota -x -c "project -s -p $MOUNT/docker-data 10000" "$MOUNT"; control_phase DATA_PROJECT_ASSIGNED; data_limit=$(quota_limit_kib 8589934592) || die data_quota_unit; xfs_quota -x -c "limit -p bhard=${data_limit} ihard=200000 10000" "$MOUNT"; state_put DATA_PROJECT 10000; state_put DATA_BYTES 8589934592; state_put DATA_INODES 200000; quota_limits_verified 10000 8589934592 200000 || die data_quota; journal DATA_QUOTA assigned; state_put PHASE QUOTA; control_phase DATA_LIMITS_VERIFIED
capacity=$(capacity_record_from_xfs) || die capacity_evidence; printf '%s\n' "$capacity" >"$ROOT/capacity.json"; chmod 0600 "$ROOT/capacity.json"; durable "$ROOT/capacity.json"; capacity_id=$(python3 -c 'import json,sys; print(json.loads(sys.stdin.read())["capacity_id"])' <<<"$capacity") || die capacity_parse; control install-capacity "$capacity" >/dev/null || die capacity_install; state_put CAPACITY_ID "$capacity_id"; journal CAPACITY "$capacity_id"
cat >"$DAEMON_JSON" <<EOF
{"data-root":"$MOUNT/docker-data","exec-root":"$RUN/exec","hosts":["unix://$SOCKET"],"storage-driver":"overlay2","features":{"containerd-snapshotter":false},"iptables":false,"bridge":"none","ip-forward":false,"ip-masq":false,"log-driver":"local","log-opts":{"max-size":"1m","max-file":"2"}}
EOF
