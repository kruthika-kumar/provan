...
def show(ctx:LocalExecutionContext,journey_id:str|None=None)->str:
    ...
    for check in readiness["checks"]:
        ...
        for record in check["record_derivations"]:
            lines.append(f"  criterion {record['criterion_id']}: {record['status']} basis={record['criterion_scoped_basis_authority']} reviewer={record['reviewer_conclusion_authority']} semantic={record['semantic_review_authority']}")
