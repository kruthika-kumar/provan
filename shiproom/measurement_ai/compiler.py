...
def build_artifacts(preparation:dict,results:dict[str,dict],verifiers:dict[str,dict]|None=None)->dict:
    ...
    for item in preparation["applicability"]["measurement"]["measurement_definition_paths"]:
        ...
        downstream.append({"path":item["path"],"requirement_ids":sorted(item["requirement_ids"]),"criterion_ids":linked,"journey_ids":sorted(item["journey_ids"]),"declared_external":item["declared_external"],"definition_state":"declared_external" if item["declared_external"] else "supplied_and_inspected" if inspected else "not_inspected","execution_state":"not_inspected","data_accuracy_state":"not_inspected","definition_authority":"deterministically_established" if item["declared_external"] else "source_verified" if inspected else "model_mapped_candidate" if not linked and not item["journey_ids"] else "not_inspected","git_object_format":selected["git_object_format"] if inspected else None,"git_blob_hash":selected["git_blob_hash"] if inspected else None,"normalized_text_hash":selected["normalized_text_hash"] if inspected else None})
...
def build_overlay(preparation:dict,results:dict,contracts:dict,instrumentation:dict,readiness:dict,recommendations:list,owner_proposals:list)->dict:
    ...
    for definition in contracts["downstream_definitions"]:
        ...
        node({"node_id":did,"node_type":"metric_definition","provenance":"measurement_ai_compiler","criterion_ids":linked,"path":definition["path"],"requirement_ids":definition["requirement_ids"],"journey_ids":definition["journey_ids"],"declared_external":definition["declared_external"],"definition_state":definition["definition_state"],"execution_state":definition["execution_state"],"data_accuracy_state":definition["data_accuracy_state"],"definition_authority":definition["definition_authority"],"git_object_format":definition["git_object_format"],"git_blob_hash":definition["git_blob_hash"],"normalized_text_hash":definition["normalized_text_hash"]})
        for cid in linked:
            if cid not in contract_nodes: continue
            contract_id=contract_nodes[cid]
            edge(contract_id,did,"uses_metric_definition",cid,definition["definition_authority"],"prepared_definition",[definition["path"]])
