...
def build_artifacts(preparation:dict,results:dict[str,dict],verifiers:dict[str,dict]|None=None)->dict:
    ...
    criterion_by_requirement={item["requirement_id"]:[] for item in preparation["authority"]["criteria"]}
    for item in preparation["authority"]["criteria"]: criterion_by_requirement.setdefault(item["requirement_id"],[]).append(item["criterion_id"])
    measurement_sources={item["path"]:item for item in source["role_sources"]["measurement"]["sources"]}
    downstream=[]
    for item in preparation["applicability"]["measurement"]["measurement_definition_paths"]:
        linked=sorted(set(item["criterion_ids"]+[
            cid for rid in item["requirement_ids"] for cid in criterion_by_requirement.get(rid,[])
        ])); selected=measurement_sources.get(item["path"])
        inspected=selected is not None and not item["declared_external"]
        downstream.append({"path":item["path"],"requirement_ids":sorted(item["requirement_ids"]),"criterion_ids":linked,"journey_ids":sorted(item["journey_ids"]),"declared_external":item["declared_external"],"definition_state":"declared_external" if item["declared_external"] else "supplied_and_inspected" if inspected else "not_inspected","execution_state":"not_inspected","data_accuracy_state":"not_inspected","declaration_authority":"deterministically_established" if item["declared_external"] or linked or item["journey_ids"] else "model_mapped_candidate","definition_content_authority":"not_inspected" if item["declared_external"] else "source_verified" if inspected else "not_inspected","definition_assertion_scope":"external_definition_declaration" if item["declared_external"] else "source_definition" if inspected else "not_inspected","git_object_format":selected["git_object_format"] if inspected else None,"git_blob_hash":selected["git_blob_hash"] if inspected else None,"normalized_text_hash":selected["normalized_text_hash"] if inspected else None})
    contracts={"schema_version":"measurement-contract.v3","release_id":source["release_id"],"contracts":canonical_contracts,"downstream_definitions":downstream,"accepted_field_projections":projections}
...
def build_overlay(preparation:dict,results:dict,contracts:dict,instrumentation:dict,readiness:dict,recommendations:list,owner_proposals:list)->dict:
    ...
    def edge(source_id,target_id,relationship,cid,direct,origin,refs,path=None):
        eid=stable_id("edge",{"source":source_id,"target":target_id,"relationship":relationship,"criterion":cid}); steps=path if path is not None else ([{"edge_id":eid,"traversal":"forward"}] if target_id==cid else node_paths[(source_id,cid)]); authority=effective_basis_class([next((item["direct_fact_authority"] for item in edges if item["edge_id"]==step["edge_id"]),direct) for step in steps]) if steps else direct
    ...
    for contract in contracts["contracts"]:
        ...
        for cid in contract["criterion_ids"]:
            e=edge(nid,cid,"governs_criterion",cid,direct,"prepared_contract",[nid]); node_paths[(nid,cid)]=[{"edge_id":e["edge_id"],"traversal":"forward"}]; node_authority[(nid,cid)]=direct
        for signal in contract["required_signals"]:
            ...
            for cid in signal["criterion_ids"]:
                e=edge(nid,sid,"requires_signal",cid,direct,"prepared_contract",[sid]); node_paths[(sid,cid)]=[{"edge_id":e["edge_id"],"traversal":"reverse"}]+node_paths[(nid,cid)]; node_authority[(sid,cid)]=effective_basis_class([direct,node_authority[(nid,cid)]])
    for definition in contracts["downstream_definitions"]:
        linked=definition["criterion_ids"]
        did=stable_id("metric_definition",definition)
        node({"node_id":did,"node_type":"metric_definition","provenance":"measurement_ai_compiler","criterion_ids":linked,"path":definition["path"],"requirement_ids":definition["requirement_ids"],"journey_ids":definition["journey_ids"],"declared_external":definition["declared_external"],"definition_state":definition["definition_state"],"execution_state":definition["execution_state"],"data_accuracy_state":definition["data_accuracy_state"],"declaration_authority":definition["declaration_authority"],"definition_content_authority":definition["definition_content_authority"],"definition_assertion_scope":definition["definition_assertion_scope"],"git_object_format":definition["git_object_format"],"git_blob_hash":definition["git_blob_hash"],"normalized_text_hash":definition["normalized_text_hash"]})
        for cid in linked:
            if cid not in contract_nodes: continue
            contract_id=contract_nodes[cid]
            edge(contract_id,did,"uses_metric_definition",cid,definition["definition_content_authority"],"prepared_definition",[definition["path"]])
    basis={item["basis_id"]:item for context in preparation["contexts"].values() for item in context["basis_registry"]}
    sources_by_path={item["path"]:item for group in source["role_sources"].values() for item in group["sources"]}
    for bid,item in sorted(basis.items()):
        prepared_source=sources_by_path.get(item.get("object_id"))
        if item["basis_type"]=="source_reference" and prepared_source is not None:
            nid=stable_id("source",{"basis":bid}); node({"node_id":nid,"node_type":"project_source_reference","provenance":"prepared_project_source","criterion_ids":item["criterion_ids"],"basis_id":bid,"path":item["object_id"],"git_object_format":prepared_source["git_object_format"],"git_blob_hash":prepared_source["git_blob_hash"],"normalized_text_hash":prepared_source["normalized_text_hash"],"direct_fact_authority":item["direct_fact_authority"]})
    conclusion_ids={}
    for role,result in sorted(results.items()):
