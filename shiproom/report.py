...
def render(release: dict, output: Path, *, events: list[dict] | None = None, audience: str = "all") -> Path:
    ...
    cards = f"<p>Selected modules: {esc(selected_summary)}</p>" + "".join(f"<article class='module {esc(m['status'])}'><p class='status'>{esc(m['status']).upper()}</p><h3>{esc(m['name'])}</h3><p>{esc(m['reason'])}</p></article>" for m in view["module_cards"])
