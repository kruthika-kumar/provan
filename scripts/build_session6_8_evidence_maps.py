...
def main() -> int:
    ...
    for requirement in inventory["requirements"]:
        ...
        parity_receipts=["session6-8-contract-parity-report.json"] if rid in {"SHARED_CONTRACT_INVENTORY","SHARED_EXECUTED_CONTRACT_PARITY"} else []
        security_receipts=["session6-8-security-receipt.json"] if rid in {"SHARED_LINK_REPARSE_SPECIAL_REJECTION","SHARED_ZERO_PROHIBITED_OPERATIONS","SHARED_TRUSTED_READS","SHARED_TRUSTED_WRITES"} else []
        wheel_receipts=["session6-8-installed-wheel-receipt.json"] if rid=="SHARED_INSTALLED_WHEEL_LIFECYCLE" else []
        ...
        execution.append({"requirement_id":rid,"production_boundary":requirement["owning_production_entrypoint"],"production_boundaries":implementation,"proof_ids":proof_ids,"canonical_artifacts":artifacts,"status":"pending_authentic_execution"})
        for row in selected:
            proofs.append({"proof_id":row["proof_id"],"requirement_id":rid,"domain":requirement["session"],"invariant":requirement["normative_behavior"],"fixture_class":row["fixture_class"],"workflow_case":row["workflow_case"],"production_functions":row["production_functions"],"artifact_queries":row["artifact_queries"],"semantic_fingerprint":row["semantic_fingerprint"],"expected_boundary_outcome":row["expected_boundary_outcome"],"expected_acceptance":row["expected_acceptance"],"expected_python_exception":row.get("expected_exception"),"expected_error_code":row.get("expected_error"),"expected_schema_rejection":False,"canonical_artifact":row["canonical_artifacts"][0],"canonical_artifacts":row["canonical_artifacts"],"minimum_record_count":row["minimum_cardinality"],"test_id":f"tests/test_session6_8_proof_execution.py::test_requirement_proof[{row['proof_id']}]","status":"pending_authentic_execution"})
        assertions=[{"assertion":"measured_requirement_value",**query} for query in by_class["valid"]["artifact_queries"]]
        claims.append({"claim_id":"claim_"+rid.lower(),"requirement_ids":[rid],"approved_semantic_hash":requirement["approved_semantic_hash"],"implementation_symbols":implementation,"positive_proof_ids":[proof_ids[0]],"near_valid_proof_ids":[proof_ids[1]],"adversarial_proof_ids":[proof_ids[2]],"artifact_queries":by_class["valid"]["artifact_queries"],"artifact_assertions":assertions,"minimum_record_counts":requirement["minimum_cardinalities"],"production_invocation_receipts":implementation,"contract_parity_receipts":parity_receipts,"security_receipts":security_receipts,"installed_wheel_receipts":wheel_receipts,"status":"pending_authentic_execution"})
    values={
