...
def main()->int:
    ...
    try:
        ...
        spy="""import json,os; import provan.modeling as m; from provan.modeling import ModelProvider,configure_provider; from provan.change_brief import explain; seen=[]; os.environ['PROVAN_MODEL_ALLOWLIST']='spy'; os.environ['PROVAN_MODEL_HOST_ALLOWLIST']='model.example.test'; m._wire_transport=lambda provider,raw,digest:(seen.append((json.loads(raw),digest)) or {'model_reviewed_implications':[],'latency_ms':1,'cost_status':'reported'}); configure_provider(ModelProvider('spy','local','1','https://model.example.test/v1')); r=explain(repo=r'%s',base='%s',head='%s',working_tree=False,brief_text='bounded',agent_claim=None,context_files=[],aliases=[],journeys=[],journey_files=[],previous_brief=None,previous_manifest=None,provider_id='spy',no_model=False); assert len(seen)==1 and r['model_usage']['calls']==1 and seen[0][1]==r['model_usage']['envelope_digest']; print('MODEL_ENVELOPE_SPY_OK')"""%(fixture,base,head)
