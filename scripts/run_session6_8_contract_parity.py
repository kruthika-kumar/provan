...
from shiproom.remediation_roadmaps import (
    authority_policy, root as remediation_root, validate_authority_policy,
...
from shiproom.review_organisation import (
    load as load_review, prepare as prepare_review, render_package,
...
def _baselines(directory: Path) -> tuple[dict[str,dict],dict[str,object],dict[str,dict]]:
    ...
    work_root=remediation_root(ctx).parent/"review-organisation"/"generations"/review_manifest["generation"]
    ...
    contest_action=_action(ctx,finding);action_fixture=copy.deepcopy(contest_action);append_action(ctx,contest_action);contest_manifest,_=load_contestation(ctx)
    management_manifest=compile_management(ctx);load_management(ctx)
