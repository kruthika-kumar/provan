...
def main()->int:
    ...
    receipt={"schema_version":"session6-8-security-receipt.v3","registry_semantic_hash":registry["approved_semantic_hash"],"records":records,"passed":len(records)==44 and all(r["typed_rejection"] and not r["underlying_adapter_called"] and not r["side_effect_observed"] for r in records)};receipt["receipt_hash"]=_sha(receipt);a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(receipt,sort_keys=True,indent=2)+"\n");return 0 if receipt["passed"] else 2
