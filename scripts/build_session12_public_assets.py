...
def main()->int:
    ...
    egress={"schema_id":"provan.foundry_model_egress_allowlist.v1","sensitivity":"PUBLIC_SAFE","provider":"openai-responses-primary","origin":"https://api.openai.com","models":[{"model":"gpt-5.6-luna","eligible_tiers":[1],"reasoning":["medium"]},{"model":"gpt-5.6-sol","eligible_tiers":[2,3],"reasoning":["high","xhigh"]}],"cases":[{"case_id":case_id,"selected_source_digests":list(digests)} for case_id,digests in sorted(FROZEN_PUBLIC_MODEL_EGRESS.items())],"arbitrary_manifest_egress":False,"operator_confirmation_required":True,"store_requested":False,"provider_retention":"PROVIDER_RETENTION_NOT_ZERO_OR_ESTABLISHED"}
