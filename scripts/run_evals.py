...
def _measurement_record(prep,cid,role):
    ...
    rungs=[{"local_id":"rung_"+name,"rung":name,"state":"established" if name in {"case_candidate","fixed_input","oracle_or_rubric","pass_condition","journey_or_criterion_linkage","prompt_or_model_binding","known_failure"} else "not_established","basis_ids":[strong["basis_id"]] if name in {"case_candidate","fixed_input","oracle_or_rubric","pass_condition","journey_or_criterion_linkage","prompt_or_model_binding","known_failure"} else [],"basis_path_ids":paths if name in {"case_candidate","fixed_input","oracle_or_rubric","pass_condition","journey_or_criterion_linkage","prompt_or_model_binding","known_failure"} else [],"limitations":[]} for name in rung_names]
...
def _measurement_ai_behavioral_evals(check) -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        prepared_info=measurement_prepare(ctx,review_mode="guided_review",review_capabilities={"schema_version":"measurement-review-capabilities.v3","executor_type":"human","reviewer_label":"eval reviewer"},applicability_path=str(app_path)); prep=load_measurement_preparation(ctx,prepared_info["preparation_id"]); mrecord=_measurement_record(prep,cid,"measurement"); arecord=_measurement_record(prep,cid,"ai_evaluation")
        signal=prep["source_packet"]["prepared_measurement_contracts"][0]["required_signals"][0]; candidate=mrecord["basis_ids"][0]; cpaths=mrecord["basis_path_ids"]; mrecord["signal_assessments"]=[{"local_id":"signal_local","signal_id":signal["signal_id"],"event_candidates":[{"local_id":"event_local","basis_ids":[candidate],"basis_path_ids":cpaths}],"property_results":[{"local_id":"property_local","property_name":"customer_id","state":"missing","basis_ids":[candidate],"basis_path_ids":cpaths}],"tests":[],"runtime_evidence":[]}]
        ...
        check("MEASUREMENT_VAGUE_INTENT_AND_CONDITIONAL_COUNT_WARNING",compiled_warning["derived_effect"]=="non_blocking_warning" and "not categorically wrong" in compiled_warning["summary"] and checks["DATA_PRIMARY_METRIC_DECISION_USEFUL"]["status"]!="gap")
        ...
        executors=({"executor_type":"human","reviewer_label":"human"},{"executor_type":"agent_harness","candidate_id":"hermes_candidate","provider_id":"configured","model_id":"configured","harness_id":"hermes","adapter_version":"1","run_id":"h"},{"executor_type":"agent_harness","candidate_id":"codex_candidate","provider_id":"configured","model_id":"configured","harness_id":"codex","adapter_version":"1","run_id":"c"})
