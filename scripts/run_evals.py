...
def _measurement_ai_behavioral_evals(check) -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        model_executor={"executor_type":"agent_harness","candidate_id":"eval_candidate","provider_id":"eval_provider","model_id":"eval_model","harness_id":"portable-eval","adapter_version":"1","run_id":"guided"}; _measurement_result(ctx,prep,"measurement",[mrecord,unresolved_record],[warning],model_executor); measurement_compile(ctx,prepared_info["preparation_id"]); _,art=load_measurement(ctx); checks={item["check_id"]:item for item in art["measurement-ai-readiness.json"]["checks"]}
        ai_app=measurement_applicability(); ai_app["ai"]["criterion_ids"]=[cid]; ai_app["ai"]["journey_ids"]=[journey]; ai_app["ai"]["linked_sources"]=[_typed_source_binding(ctx,source_path,cid,journey,subtype) for subtype in ("ai_fixed_input_definition","ai_oracle_or_rubric_definition","ai_pass_condition_definition","ai_prompt_model_binding_definition","ai_known_failure_case_definition")]; ai_path=root/"ai-applicability.json"; ai_path.write_text(json.dumps(ai_app),encoding="utf-8"); ai_info=measurement_prepare(ctx,review_mode="guided_review",review_capabilities=review_capabilities,applicability_path=str(ai_path)); ai_prep=load_measurement_preparation(ctx,ai_info["preparation_id"]); arecord=_measurement_record(ai_prep,cid,"ai_evaluation"); _measurement_result(ctx,ai_prep,"ai_evaluation",[arecord],executor=model_executor); measurement_compile(ctx,ai_info["preparation_id"]); _,ai_art=load_measurement(ctx); checks.update({item["check_id"]:item for item in ai_art["measurement-ai-readiness.json"]["checks"] if item["check_id"].startswith("AI_")})
