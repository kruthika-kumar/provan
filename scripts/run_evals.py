...
def _qualified_model_capabilities(ctx):
    ...
    candidate={"candidate_id":"eval_candidate","provider_id":"eval_provider","model_id":"eval_model","qualification_receipt_path":str(path)}
...
def _expert_verifier_eval(ctx,app,cid,app_path,disposition):
    expert=json.loads(json.dumps(app)); expert["ai"]["criterion_ids"]=[]; fields=expert["measurement"]["contracts"][0]["fields"]; fields.update({name:{"value":value,"state":"owner_confirmed"} for name,value in {"decision_owner":"product owner","decision_timing":"launch review","decision_rule_or_interpretation":"compare assigned groups","unit_of_observation":"customer","eligible_population":"invited customers","observation_window":"14 days","inference_intent":"causal_experiment"}.items()}); fields["decision_use_case"]={"value":"causal_experiment","state":"owner_confirmed"}; app_path.write_text(json.dumps(expert),encoding="utf-8")
...
def _measurement_ai_behavioral_evals(check) -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        root=measurement_root(ctx)/"inputs"; root.mkdir(parents=True); app=measurement_applicability(); app["measurement"]["criterion_ids"]=[cid]; app["measurement"]["contracts"]=[{"local_id":"launch_metric","journey_id":journey,"criterion_ids":[cid],"fields":{"decision_question":{"value":"Did onboarding improve?","state":"owner_confirmed"},"decision_use_case":{"value":"comparative_noncausal_review","state":"owner_confirmed"},"intended_outcome":{"value":"Customers onboard","state":"owner_confirmed"},"unit":{"value":"customers","state":"owner_confirmed"},"eligible_population":{"value":"invited customers","state":"owner_confirmed"},"numerator":{"value":"total onboarded customers","state":"owner_confirmed"},"guardrails":{"value":"not applicable for this bounded decision","state":"owner_confirmed"},"success_condition":{"value":"customer onboarded","state":"owner_confirmed"},"failure_condition":{"value":"customer abandons","state":"owner_confirmed"}},"metric_roles":["outcome"],"required_signals":[{"name":"customer_onboarded","required_properties":["customer_id"]}]}]; app_path=root/"applicability.json"; app_path.write_text(json.dumps(app),encoding="utf-8")
