...
def _measurement_result(ctx,prep,role,records,recommendations=None,executor=None):
    ...
    value={"schema_version":"measurement-result.v2" if role=="measurement" else "ai-evaluation-result.v2","role_id":role,"role_version":"2.0.0","preparation_id":work["preparation_id"],"work_order_id":work["work_order_id"],"base_graph_semantic_hash":work["inputs"]["graph_semantic_hash"],"resolved_review_mode":work["resolved_review_mode"],"records":records,"recommendations":recommendations or [],"assumptions":[],"limitations":[]}
...
def _measurement_record(prep,cid,role):
    bases=[item["basis_id"] for item in prep["contexts"][role]["basis_registry"] if cid in item["criterion_ids"]]
    common={"local_id":"record_"+role,"criterion_id":cid,"scope_state":"applicable","disposition":"assessed","uncertainty":"bounded","basis_ids":bases[:1],"basis_path_ids":[],"conclusion_evidence_class":"model_reviewed","semantic_review_authority":"model_reviewed_with_curated_guidance","summary":"Bounded portable assessment.","gaps":[]}
    if role=="measurement":
        ...
        return {**common,"journey_ids":[],"contract_updates":[],"signal_assessments":[],"metric_dimensions":[{"dimension":name,"state":"adequate","rationale":"Bounded prepared context."} for name in dims]}
    maturity={name:"established" for name in ("case_candidate","fixed_input","oracle_or_rubric","pass_condition","journey_or_criterion_linkage","prompt_or_model_binding","known_failure")}; maturity.update({"supplied_execution_result":"not_established","deterministically_validated_result":"not_established","production_trace_linkage":"not_established"})
    return {**common,"ai_maturity":maturity,"claims":[{"claim_id":"claim_model_quality","statement":"The model is correct.","presented_as_proof":True,"basis_ids":[],"basis_path_ids":[]}],"observability_candidates":[]}
...
def _measurement_ai_behavioral_evals(check) -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        root=measurement_root(ctx)/"inputs"; root.mkdir(parents=True); app=measurement_applicability(); app["measurement"]["criterion_ids"]=[cid]; app["ai"]["criterion_ids"]=[cid]; app["measurement"]["contracts"]=[{"local_id":"launch_metric","journey_id":journey,"criterion_ids":[cid],"fields":{"decision_question":{"value":"Did onboarding improve?","state":"owner_confirmed"},"decision_use_case":{"value":"comparative_noncausal_review","state":"owner_confirmed"},"intended_outcome":{"value":"Customers onboard","state":"owner_confirmed"},"unit":{"value":"customers","state":"owner_confirmed"},"eligible_population":{"value":"invited customers","state":"owner_confirmed"},"numerator":{"value":"total onboarded customers","state":"owner_confirmed"},"success_condition":{"value":"customer onboarded","state":"owner_confirmed"},"failure_condition":{"value":"customer abandons","state":"owner_confirmed"}},"metric_roles":["outcome"],"required_signals":[{"name":"customer_onboarded","required_properties":["customer_id"]}]}]; app_path=root/"applicability.json"; app_path.write_text(json.dumps(app),encoding="utf-8")
        prepared_info=measurement_prepare(ctx,review_mode="guided_review",review_capabilities={"schema_version":"measurement-review-capabilities.v2","executor_type":"human","reviewer_label":"eval reviewer"},applicability_path=str(app_path)); prep=load_measurement_preparation(ctx,prepared_info["preparation_id"]); mrecord=_measurement_record(prep,cid,"measurement"); arecord=_measurement_record(prep,cid,"ai_evaluation")
        signal=prep["source_packet"]["prepared_measurement_contracts"][0]["required_signals"][0]; candidate=mrecord["basis_ids"][0]; mrecord["signal_assessments"]=[{"signal_id":signal["signal_id"],"event_candidate_basis_ids":[candidate],"property_results":[{"property_name":"customer_id","state":"missing","basis_ids":[candidate]}],"test_basis_ids":[],"runtime_basis_ids":[]}]
        ...
        warning={"local_id":"count_warning","criterion_id":cid,"recommendation_class":"research_backed_warning","summary":"Opportunity volume may change; the count is not categorically wrong.","basis_ids":mrecord["basis_ids"],"basis_path_ids":[],"guidance_rule_ids":["MEAS_COUNT_002"],"exception_dispositions":exceptions,"requested_effect":"non_blocking_warning","abstained":True,"automatic_replacements":[]}
        ...
        check("MEASUREMENT_VAGUE_INTENT_AND_CONDITIONAL_COUNT_WARNING",compiled_warning["requested_effect"]=="non_blocking_warning" and "not categorically wrong" in compiled_warning["summary"] and checks["DATA_PRIMARY_METRIC_DECISION_USEFUL"]["status"]!="gap")
